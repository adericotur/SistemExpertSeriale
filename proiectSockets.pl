...
pornire :-                  %predicat principal
retractall(interogat(_)),%sterge clauzele predicatelor dinamice
retractall(fapt(_,_,_)),  % av-  predicat varsta-val ,cunostintele sist expert ,factor de certirtudine, Istoric --- utiliz sau o  lista de reguli
repeat, %repeta pana cand se introduce o val cunoscuta
write('Introduceti una din urmatoarele optiuni: '),
nl,nl,
write(' (Incarca Consulta Reinitiaza  Afisare_fapte  Cum   Iesire) '),
nl,nl,write('|: '),citeste_linie([H|T]), % pune in lista cuvintele
executa([H|T]), H == iesire. % primeste ce a returnat citeste_linie
...
determina(Atr) :-
realizare_scop(av(Atr,_),_,[scop(Atr)]),!. %pt istoric  scop[Atr]
...
fapt(Scop,FC,_), !. %verfica daca nu stie deja

realizare_scop(av(Atr, _),FC,Istorie) :- fapt(av(Atr, 'nu_conteaza'),FC,Istorie),!.
...
de_la_utiliz(X,Istorie,Lista_opt) :-
	repeat,write(': '),citeste_linie(X), %repeat pt ca poate nu da optiune valida
...
proceseaza_raspuns([X,fc,FC],_,Lista_opt):-
	member(X,Lista_opt),float(FC). %daca vrem sa ii dam noi si FC
...
incarca(F) :- % fisierul contine reguli si intrebari , citirea datelor din fisier si incarcarea lor ,scop interogabil,
...
proceseaza(L) :-
trad(R,L,[]),assertz(R), !.  % p(L, []):-np(L,L1),vp(L1,L2) unde  L1 = miauna pe campii si L2= sf_prop
%p--> np,vp,[sf_prop] daca toate prop se termina cu sf_prop,np= pisica frumoasa, vp= miauna pe campii, np--> n,adj, n-->[pisica], vp--> v,compl
trad(scop(X)) --> [scop, '/', X, '\\'].



trad(interogabil(Atr,M,P)) -->
[intrebare ,':'], [enunt,'/',P,'\\'] ,[atribut, '/',Atr,'\\'],lista_optiuni(M).

%trad(regula(N,premise(Daca),concluzie(Atunci,F))) --> identificator(N),daca(Daca),atunci(Atunci,F).
trad(regula(N,concluzie(Atunci,F),premise(Daca))) --> identificator(N),atunci(Atunci,F),daca(Daca).
...
%afiseaza(P,P) -->  [].%daca  nu avem intrebare ,ex culoare
...
lista_premise([Daca]) --> propoz_premise(Daca).

lista_premise([Prima|Celalalte]) --> propoz_premise(Prima),lista_premise(Celalalte).

%lista_premise([Prima|Celalalte]) --> propoz(Prima),[','],lista_premise(Celalalte).
%trebuie sa facem propoz(premise), propoz(concluzii)

atunci(Atunci,FC) --> [concluzie],propoz_concluzii(Atunci),[f_c, '/'],[FC],['\\'].

%atunci(Atunci,100) --> propoz(Atunci).



propoz_premise(not av(Atr,da)) --> ['/', not, Atr, '\\'].

propoz_premise(av(Atr,Val)) -->[Atr,'/',Val, '\\'].

propoz_premise(av(Atr,da))--> ['/', Atr,'\\'].


propoz_concluzii(not av(Atr,da)) --> ['/', not, Atr, '\\'].

propoz_concluzii(av(Atr,Val)) -->[Atr, '[', Val,']'].

propoz_concluzii(av(Atr,da))--> ['/', Atr, '\\'].
...
rest_cuvinte_linie(Car,[]) :-(Car==13;Car==10), !. %  linie noua
...
HH is H-48,N is HH*Aux+NN.  % aici am ramas
...
Caracter_modificat is Caracter+32;% din lit mari le face mici ca sa poti da Incarca sau incarca etc
...
citeste_cuvant(_,Cuvant,Caracter1) :-     %daca e ceva ce nu ma intereseaza
get_code(Caracter),
citeste_cuvant(Caracter,Cuvant,Caracter1).


caracter_cuvant(C):-member(C,[44,59,58,63,33,46,41,40,47,91,92,93]). % ca sa vada caracterele < > (r(<10>)) trebuie sa le adaugam
