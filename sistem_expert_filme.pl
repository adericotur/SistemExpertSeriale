...
%scopuri_princ :- 
%scop(Atr), setof(st(FC, av(Atr, Val)), Istorie^(determina(Atr), fapt(av(Atr, Val), FC, Istorie)), L )-> afis_solutii_lista(L); write('Nu am solutii!').
...
afis_regula(N) :-
	...
	scrie_lista(['lista premise','@','(']),
	scrie_lista_premise(Lista_premise),
	scrie_lista([')','concluzie','@']),
	transformare_concluzie(Scop,Scop_tr),
	append(['   '],Scop_tr,L1),
	FC1 is integer(FC),
	append(L1,[FC1],LL),
	scrie_lista(LL),nl.
...
transformare_premise(av(A,da),['@',A,'@']) :- !.

transformare_premise(not av(A,da), ['@','!',A,'@']) :- !.

transformare_premise(av(A,nu),['@','!',A,'@']) :- !.

transformare_premise(av(A,V),[A,'@',V,'@']).


transformare_concluzie(av(A,da),['@',A]) :- !.

transformare_concluzie(not av(A,da), ['@','!',A]) :- !.

transformare_concluzie(av(A,nu),['@','!',A]) :- !.

transformare_concluzie(av(A,V),[A,'@','=',V]).
...
citeste_opt(Stream,X,Optiuni,Istorie) :-
	...
	de_la_utiliz(Stream,X,Istorie,OptAux).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
lista_premise([Daca]) --> propoz_premise(Daca),[')',concluzie,'@','('].
...
atunci(Atunci,FC) --> propoz_concluzii(Atunci),[',',fc,'@'],[FC,'@',')'].

atunci(Atunci,100) --> propoz_concluzii(Atunci).


propoz_premise(not av(Atr,da)) --> ['@','!',Atr,'@'], !.
...
propoz_concluzii(not av(Atr,da)) --> ['@','!',Atr], !.
